...
class AdminController extends Controller
{
    ...
    public function storeJobfair(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            // Buat hash dari nama file asli (atau dari timestamp untuk variasi)
            $image = Hash::make($file->getClientOriginalName());

            // Bersihkan karakter hash yang tidak diizinkan dalam nama file
            $image = str_replace(['$', '/', '.'], '', $image);

            // Tambahkan ekstensi asli pada nama file yang sudah di-hash
            $image .= '.' . $file->getClientOriginalExtension();

            $file->move(public_path('assets'), $image);
    ...
    public function showSurveyById(Survey $survey)
    {

        return view('print-out.survey-by-id', compact('survey'));
    }
    // End Survey Function
